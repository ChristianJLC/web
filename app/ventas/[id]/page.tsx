...
type VentaDetalle = {
    ...
    items: Array<{
        ...
        cantidad: number;
        precioUnit: any;              // string (Decimal)
        descuento: any;               // string (Decimal)
...
export default function VentaDetallePage() {
    ...
    const [data, setData] = useState<VentaDetalle | null>(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                const res = await fetch(`/api/ventas/${params.id}`, { cache: "no-store" });
                if (!res.ok) {
                    const j = await res.json().catch(() => ({}));
                    throw new Error(j?.error || "No se pudo cargar la venta");
                }
                const j = (await res.json()) as VentaDetalle;
                if (alive) setData(j);
            } catch (e: any) {
    ...
    const fmtDate = (iso?: string) =>
        iso
            ? new Date(iso).toLocaleDateString("es-PE", {
                ...
                month: "short",
                day: "2-digit",
                timeZone: "UTC", // 🔥 evita que se reste un día
            })
            : "—";


    // Totales
    const bruto = (data?.items ?? []).reduce(
        (acc, it) => acc + Number(it.precioUnit ?? 0) * Number(it.cantidad ?? 0),
        0
    );
    const descTotal = (data?.items ?? []).reduce(
        (acc, it) => acc + Number(it.descuento ?? 0) * Number(it.cantidad ?? 0),
        0
    );
                    ...
                    <>
                        ...
                        <section className="rounded-2xl border border-white/10 bg-white/10 p-5 backdrop-blur">
                            <div className="grid gap-3 md:grid-cols-3">
                                ...
                                <Field label="Cliente" value={data.nombreCliente || "—"} />
                        ...
                        <section className="rounded-2xl border border-white/10 bg-white/10 p-0 overflow-hidden">
                                ...
                                <table className="min-w-full text-sm">
                                    <thead className="bg-white/10 text-white/80">
                                        <tr>
                                            ...
                                            <Th className="text-right">Desc. unit.</Th>
                                        ...
                                        {data.items.length === 0 ? (
                                            ...
                                            data.items.map((it) => (
                                                <tr key={it.id} className="border-t border-white/10 hover:bg-white/5">
                                                    ...
                                                    <Td right>{fmtMoney(it.precioUnit)}</Td>
                                                    <Td right>{fmtMoney(it.descuento)}</Td>
                            ...
                            <div className="flex flex-col gap-1 p-4 items-end text-sm">
                                <div className="flex gap-6">
                                    <span className="opacity-80">Bruto:</span>
                                    <span className="font-medium">{fmtMoney(bruto)}</span>
                                </div>
                                <div className="flex gap-6">
                                    <span className="opacity-80">Descuentos:</span>
                                    <span className="font-medium">- {fmtMoney(descTotal)}</span>
                                </div>
